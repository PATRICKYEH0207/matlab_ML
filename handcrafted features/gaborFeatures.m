function featureVector = gaborFeatures(File, FileName ,gaborArray,d1,d2,dimension)
...
%     end
% end
[Size,y]=size(featureVector);
max=u*v;
SizeOne=Size/max;
SizeTwo=SizeOne;
FFile = fopen('.\GaborB_originalG.csv', 'a');
fprintf(FFile, '%s, ', FileName{1,1});
for x=1:max  
    fprintf(FFile, '%2.5f, ',featureVector(SizeOne));
